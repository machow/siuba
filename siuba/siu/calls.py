...
# TODO: MetaArg should be a subclass of this?
class FormulaArg(MetaArg):
    def __init__(self, func, *args, **kwargs):
        self.func = func
        self.args = tuple()
        self.kwargs = {}
    ...
    def __repr__(self):
        return f"FormulaArg({repr(self.func)})"

    def __call__(self, x: FormulaContext):
        if isinstance(x, FormulaContext):
            return x[self.func]

        raise TypeError(
            f"The formula object {self.func} must receive a FormulaContext when called."
        )
